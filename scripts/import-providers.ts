...
const BNETZA_URL =
    'https://www.bundesnetzagentur.de/DE/Fachthemen/ElektrizitaetundGas/HandelundVertrieb/Lieferantenanzeige/DL/EnergielieferantenListe.pdf?__blob=publicationFile&v=34';
...
async function importProviders() {
    try {
        ...
        await query(
            "DELETE FROM providers WHERE name LIKE '%Strom%' OR name LIKE '%Gas%' OR LENGTH(name) < 3 OR name LIKE 'Stand %'"
        );
        ...
        for (const page of pdfData.Pages) {
            ...
            for (const [y, rowItems] of rows) {
                ...
                if (
                    nameItem &&
                    nameItem.text.length > 2 &&
                    !nameItem.text.includes('Stand') &&
                    !nameItem.text.includes('Liste der')
                ) {
