...
- [ ] **Step 3: Write minimal implementation**
    ...
    export function generatePDFHtml(providers: any[], year: number): string {
        const avgRenewable =
            providers.length > 0
                ? Math.round(
                      providers.reduce((sum, p) => sum + (p.last_renewable_percentage ?? 0), 0) / providers.length
                  )
                : 0;
...
- [ ] **Step 2: Add checkbox fields & modal connection in DashboardClient.tsx**
    ...
    - Add check handler:
        ...
        const toggleSelectProvider = (id: number) => {
            setSelectedProviderIds((prev) => (prev.includes(id) ? prev.filter((item) => item !== id) : [...prev, id]));
        };
